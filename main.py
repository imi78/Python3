...
class Grid(Widget):  # this is accessible by "root." not by "Grid."
    ...
    def btn(self):  # function for conversion 
        self.amount = int(self.amount.text)
        if self.fromCurr in rates and self.toCurr in rates and self.amount.isdigit():
          initial_amount = int(self.amount)
          amount = self.amount / rates[self.fromCurr]  # calculates the base currency (EUR) rates
        ...
        print(f"Success!  {self.amount.text}")
